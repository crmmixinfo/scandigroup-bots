...
const T = {
  ...
  uz: {
    chooseLang: 'Выберите язык / Tilni tanlang:',
    welcome: "👋 Salom! Xodim izlayapmiz.\n\nQisqa ariza — «Boshlash» tugmasini bosing.",
    start: '🚀 Boshlash',
...
bot.action(/lang_(ru|uz)/, async (ctx) => {
  ...
  await ctx.editMessageText(T[lang].welcome, Markup.inlineKeyboard([
    [Markup.button.callback(T[lang].start, 'start_apply')]
  ]));
});
...
bot.on('text', async (ctx) => {
  ...
  if (step === 'birth') {
    if (!/^\d{1,2}[.\/\-]\d{1,2}[.\/\-]\d{4}$/.test(text)) return ctx.reply(T[lang].invalid);
...
bot.on('voice', async (ctx) => {
  ...
  if (process.env.HR_CHAT_ID) {
    bot.telegram.sendMessage(process.env.HR_CHAT_ID,
  ...
  await ctx.reply(T[lang].shareLink, Markup.inlineKeyboard([
    [Markup.button.url('✈️ Telegram orqali ulashish',
      `https://t.me/share/url?url=https://t.me/${ctx.botInfo.username}&text=${encodeURIComponent('Ish bor! Ariza qoldiring 👆')}`)]
  ]));
